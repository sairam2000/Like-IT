...
const Home = ()=>{
    ...
    const {state} = useContext(Usercontest);
    return(
        ...
        <div style={{padding:20}}>
            {
                ...
                <h5><Link to= {'/Profile/'+item.postedby._id}>{item.postedby.name} </Link>{(item.postedby._id === state._id)&&<i  className="material-icons right" onClick={()=>{deletePost(item._id)}} style={{cursor:"pointer",color:"red"}}>delete</i>}</h5>
